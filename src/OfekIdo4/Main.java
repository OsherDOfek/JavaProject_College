...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		// part 3: on startup, load the college from the binary file if it exists
		College c1 = College.loadFromFile();
		if (c1 != null) {
			System.out.println("Loaded saved college: " + c1.getCollege_name());
		} else {
			System.out.println("Enter college name: ");
			c1 = new College(s.next());
		}
		...
		while (choice != 0) {
		    System.out.println("Choose an option: \n"
		    	...
		    	+ "9 ---> show the averege salary of all lecturers in a specific department\n"
		   		...
		   		+ "12 ---> compare Doctors/Professors by the number of papers they published\n"
		    ...
		    switch (choice) {
		        case 1: {
		            ...
		            try {
		            	College.checkValidName(lec_name);
		            }
		        ...
		        case 2: {
		            ...
		            // part 1: the committee's degree level is decided here, at creation time
		        ...
		        case 3: {
		            ...
		            if (com_choice < 1 || com_choice > c1.getCom().size()) {
		                System.out.println("Invalid input!");
		                break;
		            }
		            if (c1.getLec().isEmpty()) {
		            ...
		            if (lec_choice < 1 || lec_choice > c1.getLec().size()) {
		                System.out.println("Invalid input!");
		                break;
		            }
		            Committee<Lecturer> comm = c1.getCom().get(com_choice - 1);
		            Lecturer toAdd = c1.getLec().get(lec_choice - 1);
		            try {
		            	comm.add_com_mem(toAdd);
		            	System.out.println("Member added successfully");
		            }
		            catch(AlreadyCommitteeMemberException | IncompatibleMemberDegreeException e) {
		            	System.out.println(e);
		            }
		            break;
		        }
		        ...
		        case 4: {
		            ...
		            if (com_num < 1 || com_num > c1.getCom().size()) {
		                System.out.println("Invalid input!");
		                break;
		            }
		            if (c1.getLec().isEmpty()) {
		            ...
		            if (lec_num < 1 || lec_num > c1.getLec().size()) {
		                System.out.println("Invalid input!");
		                break;
		            }
		            Lecturer newHead = c1.getLec().get(lec_num - 1);
		            try {
		            	c1.getCom().get(com_num - 1).setHead_of_com(newHead);
		        ...
		        case 5: {
		            ...
		            printComList(c1);
		            int com = s.nextInt();
		            if (com < 1 || com > c1.getCom().size()) {
		                System.out.println("Invalid input!");
		                break;
		            }
		            Committee<Lecturer> comm = c1.getCom().get(com - 1);
		            if (comm.getMembers().isEmpty()) {
		            ...
		            String lec_to_remove = comm.getMembers().get(lec - 1).getName();
		            try {
		        ...
		        case 7: {
		            ...
		            if (dep_choice < 1 || dep_choice > c1.getDep().size()) {
		                System.out.println("Invalid input");
		                break;
		            }
		            if (c1.getLec().isEmpty()) {
		            ...
		            if (lec_number < 1 || lec_number > c1.getLec().size()) {
		                System.out.println("Invalid input");
		                break;
		            }
		            Lecturer chosen = c1.getLec().get(lec_number - 1);
		            ...
		            Department d = c1.getDep().get(dep_choice - 1);
		            try {
		        ...
		        case 10: {
		            if (c1.getLec().isEmpty())
		                ...
		                for (int i = 0; i < c1.getLec().size(); i++)
		                    System.out.println(c1.getLec().get(i).toString());
		            break;
		        }
		        ...
		        case 11: {
		            if (c1.getCom().isEmpty())
		                ...
		                for (int i = 0; i < c1.getCom().size(); i++)
		                    System.out.println(c1.getCom().get(i).toString());
		            break;
		        }
		        ...
		        case 12: {
		        	...
		        	if(cmp_choice < 1 || cmp_choice > c1.getLec().size()) {
		        		System.out.println("Wrong input!");
		        		break;
		        	}
		        	if(!(c1.getLec().get(cmp_choice - 1) instanceof Doctor)) {
		        		System.out.println("Cant compare him/her/them");
		        		break;
		        	}
		        	Doctor first_lec = (Doctor) c1.getLec().get(cmp_choice - 1);
		        	...
		        	Doctor second_lec = (Doctor) c1.getLec().get(cmp_choice - 1);

		        	int cmp = first_lec.compareTo(second_lec);
		        	if(cmp < 0)
		        		System.out.println(second_lec.getName() + " has more papers than " + first_lec.getName());
		        	else if(cmp > 0)
		        		System.out.println(first_lec.getName() + " has more papers than " + second_lec.getName());
		        	else
		        ...
		        case 13: {
		        	...
		        	Committee<Lecturer> toCompare1 = c1.getCom().get(com_choice - 1);
		        	...
		        	Committee<Lecturer> toCompare2 = c1.getCom().get(com_choice - 1);
		        ...
		        case 14: {
		        	...
		        	String com_name = c1.getCom().get(com_choice - 1).getName();
		        	try {
		        		c1.duplicate_committee(com_name);
		        		System.out.println("Committee duplicated successfully");
		        	}
		        	catch(CommitteeAlreadyExistsException | InvalidDegreeException | AlreadyCommitteeMemberException | IncompatibleMemberDegreeException e) {
		        		System.out.println(e);
		        	}
		        	break;
		        }
		        ...
		        case 15: {
		        	...
		        	Lecturer chosen = c1.getLec().get(lec_choice - 1);
		        ...
		        case 0: {
		        	// part 3: on exit, save all the info to a binary file
		        	try {
		        		c1.saveToFile();
		        		System.out.println("Data saved. Exiting...");
		        	} catch (IOException e) {
		        		System.out.println("Error: couldn't save data to file");
		        	}
		            break;
				}
	...
	private static void printComList(College c) {
		for (int i = 0; i < c.getCom().size(); i++)
			System.out.println((i + 1) + "---> " + c.getCom().get(i).getName());
	}

	private static void printLecList(College c) {
		for (int i = 0; i < c.getLec().size(); i++)
			System.out.println((i + 1) + "---> " + c.getLec().get(i).getName());
	}

	private static void printDepList(College c) {
		for (int i = 0; i < c.getDep().size(); i++)
			System.out.println((i + 1) + "---> " + c.getDep().get(i).getDep_name());
	}

	private static void printMembers(Committee<?> comm) {
		for (int i = 0; i < comm.getMembers().size(); i++)
			System.out.println((i + 1) + "---> " + comm.getMembers().get(i).getName());
	}
}
